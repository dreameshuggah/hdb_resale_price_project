...
def predict(request: PredictRequest):
    ...
    try:
        encoded = {
            "flat_type_rank":     flat_type_map[request.flat_type],
            "region_ura_rank":    region_map[request.region_ura],
            "town_rank":          town_map[request.town],
            "storey_range_rank":  storey_map[request.storey_range],
            "flat_model_rank":    flat_model_map[request.flat_model],
            ...
            "resale_price_index": request.resale_price_index,
        }

        features_df = pd.DataFrame([encoded])[FEATURE_COLS]
        prediction: float = float(_model.predict(features_df)[0])
